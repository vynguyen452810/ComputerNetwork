...
class Switch(StpSwitch):
    ...
    def __init__(self, idNum: int, topolink: object, neighbors: list):
        ...
        self.root = idNum    # a variable to store the switch ID that this switch sees as the root
        self.distanceToRoot = float('inf') # a variable to store the distance to the switch’s root
        self.activeLinks = [] # links to neighbors that are in the spanning tree
        self.neighbor = None # a variable to keep track of which neighbor it goes through to get to the root
